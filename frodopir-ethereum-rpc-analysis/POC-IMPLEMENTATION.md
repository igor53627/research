...
- ✅ ~100ms query latency (acceptable for RPC use)
...
│         ▼                                                         │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  SERVICE 6: Ambire Wallet (React + Vite)                 │   │
│  │  - FrodoPIRProvider integration                          │   │
│  │  - Privacy Mode UI                                       │   │
│  │  Port: 5173                                              │   │
│  └────┬───────────────────────────┬───────────────────────┬─┘   │
│       │                           │                       │      │
│       │ PIR Query                 │ Hint Download         │ Fallback RPC
│       ▼                           ▼                       ▼      │
│  ┌─────────────────┐       ┌─────────────────┐    ┌──────────┐ │
│  │ SERVICE 4:      │       │ SERVICE 5:      │    │ Ethereum │ │
│  │ PIR Server      │       │ CDN Mock        │    │ RPC      │ │
│  │ (Express)       │       │ (nginx)         │    │ (Public) │ │
│  │ Port: 3000      │       │ Port: 8080      │    │          │ │
│  └────┬────────────┘       └─────────────────┘    └──────────┘ │
│       │                                                          │
│       │ Uses                                                     │
...
│  ┌───────────────────────────────────────────────┐              │
│  │   SHARED VOLUME: /data                        │              │
│  │   ├── database.bin (4,096 entries × 128 bytes)│              │
│  │   ├── address-mapping.bin (4,096 × 20 bytes) │              │
│  │   └── hint.bin (~12 MB compressed)            │              │
│  └───────────────────────────────────────────────┘              │
│         ▲                      ▲                                 │
...
│   │   ├── frodopir-lib.js        # PIR computation wrapper
│   │   └── config.json
│   │
│   ├── cdn-mock/
│   │   ├── Dockerfile
│   │   └── nginx.conf             # Serve /data/hint.bin
│   │
│   └── ambire-wallet/
...
│       ├── src/
│       │   ├── providers/
│       │   │   ├── FrodoPIRProvider.js  # NEW: Custom provider
│       │   │   └── index.js             # Export provider
│       │   ├── components/
│       │   │   └── PrivacyMode.jsx      # NEW: UI toggle
│       │   └── lib/
│       │       └── frodopir-client.js   # NEW: PIR client logic
...
**Purpose**: Extract state from Ethereum Mock → create FrodoPIR database
...
if [ $HINT_SIZE -lt $EXPECTED_MIN ] || [ $HINT_SIZE -gt $EXPECTED_MAX ]; then
  echo "⚠️  WARNING: Hint size outside expected range (10-15 MB)"
fi
...
console.log(`✅ Database loaded: ${database.length} bytes`);
console.log(`✅ Hint loaded: ${hint.length} bytes`);
...
app.post('/query', async (req, res) => {
  ...
  try {
    ...
    console.log(`📨 Received PIR query: ${query.length} bytes`);

    // Compute PIR response using FrodoPIR algorithm
    const response = computeResponse(query, database, hint);

    const latency = Date.now() - startTime;
    console.log(`✅ Computed response in ${latency}ms: ${response.length} bytes`);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 PIR Server listening on port ${PORT}`);
});
```
...
function computeResponse(query, database, hint) {
  ...
  console.log('⚠️  Using mock PIR response (implement actual FrodoPIR)');
...
export class FrodoPIRProvider extends BaseProvider {
  ...
  async ensureHintLoaded() {
    ...
    console.log('📥 Downloading PIR hint...');
  ...
  async pirGetBalance(address) {
    ...
    const latency = Date.now() - startTime;
    console.log(`✅ PIR query completed in ${latency}ms (private!)`);
...
export class FrodoPIRClient {
  ...
  async generateQuery(address) {
    ...
    console.log(`🔐 Generating PIR query for ${address} (private!)`);
  ...
  async decryptResponse(responseBuffer) {
    ...
    console.log('🔓 Decrypting PIR response...');
...
if (USE_PIR) {
  ...
  console.log('✅ Privacy Mode enabled (using FrodoPIR)');
} else {
  ...
  return (
    <div className="privacy-mode-toggle">
      <label>
        ...
        />
        🔐 Privacy Mode (FrodoPIR)
      </label>

      {enabled && (
        <div className="privacy-info">
          ✅ Your balance queries are private.<br/>
...
# 5. Check browser console: "✅ PIR query completed in 95ms (private!)"
```
...
# Expected output:
# 📨 Received PIR query: 32768 bytes
# ✅ Computed response in 89ms: 2048 bytes
# (NO address logged = privacy working!)

# Compare with fallback RPC logs (if Privacy Mode off):
# Request: eth_getBalance("0x1234...") ← ADDRESS LEAKED!
```
...
echo "🧪 Testing FrodoPIR PoC..."

# Test 1: Services running
echo "Test 1: Checking services are healthy..."
docker-compose ps | grep "Up" | wc -l | grep -q "4" && echo "✅ All services running"

# Test 2: PIR server health
echo "Test 2: PIR server health..."
curl -f http://localhost:3000/health && echo "✅ PIR server healthy"
...
[ $SIZE -gt 10000000 ] && echo "✅ Hint downloaded ($SIZE bytes)"
...
[ $LATENCY -lt 200 ] && echo "✅ Latency acceptable (<200ms)"
